 ...
 * anteprima sessioni e UI più leggibile (senza CSS esterno).
 */
public class CorsiPanelController {
 ...
 // in CorsiPanelController
    ...
    @FXML private TableColumn<Corso, String> colStato; // aggiungi in FXML a destra di Argomento/Frequenza
    ...
    private void initialize() {
        ...
        table.getColumns().addListener((javafx.collections.ListChangeListener<? super TableColumn<Corso, ?>>) c ->
...
private void initTableColumns() {
    ...
    colStato.setCellFactory(tc -> new TableCell<Corso, String>() {
        ...
        @Override protected void updateItem(String s, boolean empty) {
            ...
            if (empty || s == null || s.isEmpty()) { setGraphic(null); return; }
            chip.setText(s);
            String bg = switch (s) {
    ...
    private void applyTableStyling() {
        ...
        Platform.runLater(() -> {
            ...
            if (headerRow != null) headerRow.setStyle("-fx-background-color:" + BG_HDR + ";");

            Node headerBg = table.lookup(".column-header-background");
            if (headerBg != null) headerBg.setStyle("-fx-background-color:" + BG_HDR + ";");

            for (Node n : table.lookupAll(".column-header")) {
                if (n instanceof Region r) {
                    r.setStyle("-fx-background-color:" + BG_HDR + "; -fx-background-insets: 0; -fx-border-color: " + GRID + "; -fx-border-width: 0 0 1 0;");
                }
            ...
            if (filler != null) filler.setStyle("-fx-background-color:" + BG_HDR + ";");
 ...
 // dopo reload() o dove preferisci
   ...
   public void setDaos(CorsoDao corsoDao, SessioneDao sessioneDao, RicettaDao ricettaDao) {
	    ...
	    // se la TableView non è ancora in scena, aspetta che venga attaccata e poi ricarica
...
private void styleDarkButtons(DialogPane pane, ButtonType okType, ButtonType cancelType) {
    ...
    private void openReportMode() {
        if (corsoDao == null) { showError("DAO non inizializzato. Effettua il login."); return; }
        String cf = corsoDao.getOwnerCfChef();
        if (isBlank(cf)) { showError("Chef non identificato. Effettua il login."); return; }
    ...
    private void onEdit() {
        ...
        final javafx.concurrent.Task<List<Sessione>> loadTask = new javafx.concurrent.Task<>() {
            @Override protected List<Sessione> call() throws Exception { return sessioneDao.findByCorso(sel.getIdCorso()); }
        };
        ...
        loadTask.setOnSucceeded(ev -> {
            ...
            try {
                ...
                if (pane.getButtonTypes().isEmpty()) pane.getButtonTypes().setAll(ButtonType.OK, ButtonType.CANCEL);
                ...
                if (res.isPresent() && res.get() != null) {
                    ...
                    final javafx.concurrent.Task<Void> replaceTask = new javafx.concurrent.Task<>() {
                        @Override protected Void call() throws Exception { sessioneDao.replaceForCorso(sel.getIdCorso(), result); return null; }
                    };
    ...
    private CustomMenuItem createFilterRow(String title, String value, String svgPath,
                                           ...
                                           java.util.function.Consumer<Button> clearButtonSink,
                                           java.util.function.Consumer<Label> valueLabelSink) {
        ...
        btnX.setOnAction(e -> { if (onClearClick != null) onClearClick.run(); });
        btnX.setStyle("-fx-background-color: transparent; -fx-text-fill:#e9f5ec; -fx-font-size:14px; -fx-padding:0 6 0 6;");
    ...
    private CustomMenuItem createDateRow(String title, boolean isFrom) {
        ...
        dp.setConverter(new StringConverter<>() {
            ...
            @Override public LocalDate fromString(String s) { return (s == null || s.trim().isEmpty()) ? null : LocalDate.parse(s, fmt); }
        });

        Button clear = new Button("×");
        clear.setOnAction(e -> dp.setValue(null));
        clear.setStyle("-fx-background-color: transparent; -fx-text-fill:#e9f5ec; -fx-font-size:14px; -fx-padding:0 6 0 6;");
        ...
        dp.valueProperty().addListener((obs, oldV, newV) -> {
            if (isFrom) dateFrom = newV; else dateTo = newV;
            boolean on = newV != null;
            clear.setVisible(on); clear.setManaged(on);
   ...
   private void styleDatePickerInline(DatePicker dp) {
    ...
    private CustomMenuItem separatorItem() {
        ...
        line.setMinHeight(1); line.setPrefHeight(1); line.setMaxHeight(1);
 ...
 private static void applyRowStyle(TableRow<Corso> row, boolean hovered, boolean selected) {
    ...
    private static boolean matchesEqIgnoreCase(String value, String filter) {
        if (isBlank(filter)) return true; return value != null && value.equalsIgnoreCase(filter);
    }
    private static boolean matchesContainsIgnoreCase(String value, String filter) {
        if (isBlank(filter)) return true; return value != null && value.toLowerCase().contains(filter.toLowerCase());
    }
    ...
    private void styleFilterMenuButton() {
        ...
        final String TXT = "#e9f5ec";
        if (btnFilters == null) return;

        btnFilters.setStyle("-fx-background-color:" + BG + ";" + "-fx-text-fill:" + TXT + ";" + "-fx-background-radius: 10;" + "-fx-padding: 8 10;");
    ...
    private void onAssociateRecipes() {
        ...
        try {
            ...
            fx.setControllerFactory(t -> {
                ...
                catch (Exception e) { throw new RuntimeException("Errore creazione controller: " + t, e); }
            });
