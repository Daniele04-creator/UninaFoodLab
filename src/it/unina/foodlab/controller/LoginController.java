		...
		try {
				...
				registerBtn.setOnAction(e -> {
					...
					try {
						...
						if (out == ChefDao.RegisterOutcome.OK) {
							...
							catch (IOException ex) { regCtrl.showError("Registrazione completata, ma errore nel ritorno al login."); }
			...
			if (cancelBtn != null) {
				cancelBtn.setOnAction(e -> {
					try { loadLoginScene(st); }
					catch (IOException ex) { showError("Errore caricando il login."); }
				});
			}
	...
	private void enforceFullScreenLook(final Stage st) {
		Platform.runLater(() -> {
			try {
				...
				pt.setOnFinished((ActionEvent e) -> st.setAlwaysOnTop(false));
				pt.play();
