...
public class CorsoDao {
    ...
    /** Inserisce SOLO il corso (non le sessioni). Preferisci insertWithSessions() nel flusso "nuovo". */
    ...
    private long insertCorso(Connection conn, Corso c) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            // forza sempre l'owner del DAO come proprietario del corso
            ps.setString(6, ownerCfChef);
            try (ResultSet rs = ps.executeQuery()) {
                rs.next();
                long id = rs.getLong(1);
                c.setIdCorso(id);
                // allinea anche l'oggetto Chef del corso
                if (c.getChef() == null) {
                    Chef s = new Chef();
    ...
    /** Elenco argomenti distinti presenti in tabella corso (ordinati A→Z). */
    ...
    /** Elenco frequenze distinte presenti in tabella corso (ordinate A→Z). */
    ...
    private Corso mapRow(ResultSet rs) throws SQLException {
        ...
        int ns = rs.getInt("num_sessioni"); // alias coerente con SELECT
        if (rs.wasNull()) ns = 1;
        c.setNumSessioni(ns);
